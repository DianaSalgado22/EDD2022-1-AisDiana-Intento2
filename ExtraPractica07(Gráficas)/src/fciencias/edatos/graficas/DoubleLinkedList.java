...
public class DoubleLinkedList<T> implements TDAList<T>{
  ...
  private class Nodo {
     ...
     */
    public void setNext(Nodo newNodo) {
      this.next = newNodo;
    }
     ...
     */
    public void setPrev(Nodo newNodo) {
      this.prev = newNodo;
    }
  ...
  public boolean contains(T e) {
    int contador = 0;
    Nodo iterador1 = head; // Iterador #1 que inicia en cabeza.
    Nodo iterador2 = tail; // Iterador #2 que inicia en la cola.
  ...
  public Nodo getNodo(int i) throws IndexOutOfBoundsException {
    if (i >=size || i < 0) {
      System.out.println(yellow+"No se puede obtener el elemento porque el indice es invalido"+white);
      throw new IndexOutOfBoundsException();
    }
